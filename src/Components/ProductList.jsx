...
function ProductList() {
    ...
    const [showPlants, setShowPlants] = useState(false); // State to control the visibility of the plant list
   ...
   const handleCartClick = (e) => {
    ...
    setShowPlants(false); // Hide the plant list
};
...
const handleContinueShopping = (e) => {
    e.preventDefault();
    setShowCart(false);
    setShowPlants(true); // Return to plant list when continuing shopping
};
...
const addToCart = (plant) => {
    ...
    const id = `${plant.name}-${new Date().getTime()}`; // Simple unique ID based on name and timestamp
    if (existingPlant) {
        ...
        setCartItems([...cartItems, { ...plant, id, quantity: 1 }]);
    }
};
