...
function ProductList() {
   ...
   const styleA={
    ...
    // Handle clicking on the Plants category
    const handlePlantsClick = (e) => {
        e.preventDefault();
        // setShowPlants(true);
        setShowCart(false);
        const allPlants = plantsArray.reduce((acc, category) => [...acc, ...category.plants], []);
        setSelectedPlants(allPlants); // Show all plants from all categories
    };

    // Handle clicking on the Cart
    ...
    const addToCart = (plant) => {
        const updatedPlant = {
            ...plant,
            quantity: 1,  // Initialize the quantity when adding a new item
        };
        setCartItems([...cartItems, updatedPlant]);
            ...
            {!showCart ? (
                <div className="product-grid">
                    {selectedPlants.length > 0 ? (
                        ...
                        <p>Select a category to view plants.</p>
                    )}
                </div>
            ) : (
